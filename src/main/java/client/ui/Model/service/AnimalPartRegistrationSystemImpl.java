...
public class AnimalPartRegistrationSystemImpl extends Client implements AnimalPartRegistrationSystem
{
  ...
  @Override public void updateAnimalPart(AnimalPart data) throws UpdateFailedException, NotFoundException {
    ...
    try {
      ...
      AnimalPartData animalPart = AnimalPart_ToGrpc_AnimalPartData.convertToAnimalPartData(data);

      // Prompt gRPC to update the Animal:
      EmptyMessage updated = stub.updateAnimalPart(animalPart);
      ...
      if(e.getStatus().equals(INTERNAL))
        throw new UpdateFailedException("Critical Error encountered. Failed to Update AnimalPart with id '" + data.getPart_id() + "'");
    } finally {
      // Always shut down the channel after use, to reduce server congestion and 'application hanging'.
      channel.shutdown();
    }
